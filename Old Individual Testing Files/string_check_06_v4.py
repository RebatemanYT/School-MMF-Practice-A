    ...
    while loop_exit == 0:
      ...
      # ask user for desired snack and put it in lowercase
        ...
        if snack_choice == "invalid choice":
          ...
          snack_choice[0] = snack_int
          #if item doesn't have a number, set number as 1. if item has a valid number, seperate it into 2 (number / item)
          if snack_int != int():
            amount = 1
            desired_snack = snack_choice
          elif snack_int[0] >= 5:
            if snack_choice[0] <= 0:
              amount = snack_choice[0]
